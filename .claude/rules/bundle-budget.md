...
- **`entryOnly` for a deliberately code-split entry.** By default the gate sums *every* emitted chunk — correct when the whole cost lands in the initial payload. An entry that splits on purpose (a registry of `() => import(…)` behind a `name` prop, e.g. `illustration-root`) is different: its async chunks are fetched only when a consumer names one, so summing them budgets the size of the whole library instead of what any consumer pays, and the ceiling has to be raised every time the library grows — which trains people to raise it. Such an entry sets `"entryOnly": true` and is budgeted on its **initial chunk**, a number that stays flat and still fails loudly if a static import ever collapses the splitting. The gate prints how many async chunks it excluded, so an `entryOnly` budget can never quietly hide growth. Use it only for genuine lazy-registry entries — never to duck a real regression.
