...
figma:
  ...
  node_id: 412:140497
checksum: 5313fcb20b2bdb9778393741ed4074d7cc557d911692ad8575d35c9fefa0119c
created: 2026-08-05
...
- `illustration-window/illustration-window.vue` — an abstracted application window: three status dots over one of three scenario bodies (`icon` centers a glyph, `chat` stacks alternating message blocks, `website` stacks a header, a hero, and a footer block). Carries the rim light over `--bg-canvas`. Only `medium` and `large` — a 32 px window has no room for a scene.
...
- `illustration-gauge/illustration-gauge.vue` — a circular progress ring with an optional value in the middle. The arc is drawn by dashing the circumference, so any `value` renders without extra geometry.
  - Props: `value?: number` (default `100`) — share of the ring filled, 0–100, clamped; `severity?: 'success' \| 'warning' \| 'danger' \| 'info'` (default `'success'`) — which feedback role colors the arc; `label?: string` (default `''`) — text in the middle; `active?: boolean` (default `undefined` → inherits) — draws the arc in the brand color instead of its severity.
  - Events: _none_.
  - Slots: _none_.
...
- A registered asset is authored against the fixed illustration canvas (`--illustration-canvas-width` × `--illustration-canvas-height`) and places its parts at exact coordinates, so it renders at its designed size and does not follow `size`. `size` governs hand-composed illustrations.
