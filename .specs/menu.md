...
figma:
  ...
  node_id: '5993:33335'
checksum: c522ce685159dadb06804b664344695283f919d4fae8e70493b23f92f7f03202
created: 2026-08-03
...
- `menu-sub-trigger/menu-sub-trigger.vue` — The row that owns children, and a **heading for them**.
  ...
  - **The row is TWO controls, of either kind** — a **label** that references wherever the row points, and an **arrow** that reveals its children. That is the whole shape of the component: a row that owns children is still a destination, and reaching its children must not cost the reader that destination. So the root is a plain flex **box** (`relative flex h-8 w-full items-center gap-(--spacing-xxs) pr-(--spacing-xxs)`, not interactive) holding the two, each with its own surface, its own focus ring and its own hit area. Nesting is not an option — a `<button>` may not contain a `<button>` — and the split is what the reference sidebars show: one focus ring on the label, another on the arrow.
    - The **label** is a `<button>` carrying `sub.triggerId`, the icon column and the row's typography (`min-w-0 flex-1`, so it fills what the arrow leaves). It announces its activation through `click` and does **nothing else** — it neither expands nor pushes.
    - The **arrow** is an `IconButton kind="transparent" size="small"` (28 px box, 12 px glyph). With `kind="inline"` it is a rotating `pi pi-chevron-down` that toggles the sub and carries **`aria-expanded` / `aria-controls`** — they belong on the control that expands the children, not on the label, which expands nothing. With `kind="drill"` it is a static `pi pi-chevron-right` that pushes the sub onto the root's stack and carries **no** `aria-expanded` — nothing expands, so the attribute would be a lie. Its accessible name is derived, since the glyph alone says nothing: `Expand <label>` / `Collapse <label>` for an inline arrow (it names the state it moves to, which `aria-expanded` then confirms) and `Open <label> menu` for a drill one. It overrides `IconButton`'s ring offset with the menu's own (`ring-offset-1` against `--menu-ring-offset`) — both with `!`, because they conflict with utilities `IconButton` puts in the same class list, where stylesheet order rather than source order would otherwise decide. Its glyph colour goes through `iconClass` (onto the `i`) for the same reason.
    - **The arrow keys stay on the label**: `ArrowRight` expands / pushes and `ArrowLeft` collapses, from the label itself, so a keyboard reader reaches the children without first tabbing to the arrow.
    - Focus returns to the **arrow** when a drill level is popped — the control that opened it, not the label beside it, which goes somewhere else.
  ...
  - **Every trigger is a destination as well as a container.** In data-driven mode a node with children emits `navigate` `(event, node)` when its **label** is activated — of either kind, so a condensed row can point at a landing page and still own children. A node with nothing to point at simply gives the consumer nothing to route to (its own `href` / landing map is the check). The **arrow** emits nothing: revealing children is a move inside the menu, not a navigation.
  - Props: `label?: string` (default `''` — visible row text), `kind?: 'inline' | 'drill'` (default `'inline'` — whether children expand in place or replace the menu), `icon?: string` (default `''` — leading glyph; honoured for `kind="drill"` only, see above), `disabled?: boolean` (default `false` — blocks toggling and pushing; suppresses **both** controls).
  - Events: `click` `(event: MouseEvent)` — the **label** was activated. Declared rather than left to `$attrs` fallthrough: the root is a box holding two controls, so a fallthrough listener would sit on that box and fire for the arrow too, making "reveal the children" also trigger the consumer's navigation.
...
- `data-kind` values: `inline` | `drill` — on `SubTrigger` and `SubContent`. Both sit on the trigger's row **box**, which is the root of its two controls.
...
- ARIA:
  ...
  - An inline `SubTrigger`'s **arrow** carries `aria-expanded` + `aria-controls` pointing at its `SubContent` — the control that expands the children owns them; the label beside it expands nothing and carries neither. The arrow is named `Expand <label>` / `Collapse <label>`, so the state is announced twice over: by the name it moves to and by `aria-expanded`.
  - A drill `SubTrigger` carries **no** `aria-expanded` on either control — it replaces the view rather than expanding one. Its arrow is named `Open <label> menu`. On push, focus moves to the level's `Back` button and the pushed `<ul>` takes `aria-label` from the trigger's label; on pop, focus returns to the **arrow** that pushed it. A level that is not current carries **both `aria-hidden` and `inert`** while it is still mounted for the slide, so it leaves the accessibility tree and the tab order together (the pattern Kumo's `SlidingViews` uses).
