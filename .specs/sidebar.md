...
figma:
  ...
  node_id: 3735:14866
checksum: 1076c3ce003478f75df8de33f9ea79b49fb7ba471aae01eaf565349ffc19e9c6
created: 2026-05-22
...
slot holds checkboxes (a fields picker, a filter list) would collapse itself the moment a consumer
checked one. Because a native checkbox reflects `:checked` the instant it is
...
rail resizes and nothing fades. Once the pull reaches the **minimum** and the user persists left,
the content begins to **fade** while the rail keeps following the pointer. Past the collapse
threshold, release **removes** it; release before the threshold springs the rail back to the
minimum at full opacity.

**Above the minimum the panel is the rail, at the rail's width**, so a resize reflows its content
the way any width change does.

**Below the minimum the panel holds the minimum and is carried by the closing edge.** It does not
reflow further: the minimum width is the width the rail's labels were fitted to, so narrowing the
content past it truncates them — measured, `SQL Database` ellipsizes at 225px. Instead the panel
keeps `min-width` and takes an inline `translate` of `railWidth - minWidth`, which holds its
trailing edge flush with the rail's (measured gap: 0px at every sample). Leaving it at `translate:
0` instead pins the content in place while the rail's edge sweeps across it — the reading that the
content sits on a layer *beneath* the rail rather than being carried by it.

The fade runs alongside, from `1` at the minimum down to `RAIL_MIN_OPACITY` at zero width, meeting
the value `has-[>input:checked]:opacity-20` holds once the rail is closed. The inline `translate`
and `opacity` are also what hold the collapsed classes off during a peek drag.

So the gesture never shows the consumer's content at a width it was not designed for, and never
detaches it from the edge that is moving.
...
follows the pointer continuously from `max-width` down to 0 and the gesture is 1-to-1 for its whole
length. On release the minimum is restored — a rail let go under it springs back to the minimum,
and only a pull past the collapse threshold closes it.

Pinning the rail at its minimum instead costs the gesture its honesty in two ways, both measured.
The rail stops under the pointer for the last ~56px and then jumps the whole snap distance in one
frame the instant `collapsed` flips (256 → 196, in a gesture that is otherwise 4px per frame).
And whatever feedback is substituted in that band — sliding the panel, fading it — decouples the
content from the rail that contains it, which reads as the content sitting on a layer beneath the
rail rather than being the rail.
