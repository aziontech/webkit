...
- **shadcn-vue patterns** (see COMPONENT_REQUIREMENTS § 2.5-2.9):
  - **Available today** (use immediately): `data-state`/`data-disabled`/`data-orientation` on root and stateful sub-components; `VariantProps` exported as named exports (`export type ButtonKind = ...`).
  - **Conditional — pending deps not yet installed** (do NOT emulate before they land — the `validate-references.mjs` hook will block fake imports): `asChild` prop (requires a Slot helper to be added in `packages/webkit/src/composables/`); `cn` helper (requires `clsx` + `tailwind-merge` in `packages/webkit/package.json`); `<name>.figma.ts` Code Connect (requires `@figma/code-connect`); Storybook `play` functions (require `@storybook/test`). Until each dependency lands and the helper exists, register the gap in the report and skip that part. Do not invent imports.
