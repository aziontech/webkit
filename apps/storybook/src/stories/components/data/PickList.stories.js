...
const CONTROL_BUTTON_CLASS =
  'rounded-[var(--shape-button)] border border-[var(--border-default)] px-[var(--spacing-sm)] py-[var(--spacing-xxs)] text-label-md text-[var(--text-default)] hover:bg-[var(--bg-hover)] disabled:cursor-not-allowed disabled:opacity-50'

const CUSTOM_CONTROLS_SOURCE = toSfc(
  [
    "import PickList from '@aziontech/webkit/pick-list'",
    "import { ref } from 'vue'",
    '',
  ...
  [
    ...
    '  <button :disabled="!pl?.hasSelection(\'source\')" @click="pl.move(\'to-target\')">Move selected →</button>',
    '  <button :disabled="pl?.count(\'source\') === 0" @click="pl.moveAll(\'to-target\')">Move all →</button>',
    '</div>',
    '',
    '<PickList ref="pl" v-model="model" data-key="id">',
...
export const CustomControls = {
  render: () => ({
    components,
    setup() {
      const value = ref(sampleModel())
      const pl = ref()
      return { value, pl, CONTROL_BUTTON_CLASS }
    },
    template: `
      <div class="max-w-[var(--container-xl)]">
        <div class="mb-[var(--spacing-sm)] flex gap-[var(--spacing-xs)]">
          <button
            type="button"
            :class="CONTROL_BUTTON_CLASS"
            :disabled="!pl?.hasSelection('source')"
            @click="pl.move('to-target')"
          >
            Move selected →
          </button>
          <button
            type="button"
            :class="CONTROL_BUTTON_CLASS"
            :disabled="pl?.count('source') === 0"
            @click="pl.moveAll('to-target')"
          >
            Move all →
          </button>
        </div>
        <PickList ref="pl" v-model="value" data-key="id">
          <PickList.Source header="Available">
            <template #item="{ item }">{{ item.label }}</template>
          </PickList.Source>
          <PickList.Target header="Selected">
  ...
  parameters: {
    docs: {
      ...
      source: { code: CUSTOM_CONTROLS_SOURCE },
      description: {
        story:
          'Custom move controls rendered **outside** the PickList, driven by a template ref — the root exposes `move` / `moveAll` / `hasSelection` / `count`, so `PickList.Controls` is omitted entirely. The buttons enable/disable reactively as you select items.'
      }
    }
  }
}
