...
const REQUIREMENTS_INVALID_TEMPLATE = `<div class="flex flex-col gap-6 w-[320px]">
  <FieldPassword
    v-model="password"
    label="Password"
    placeholder="Type your password"
    autocomplete="new-password"
    invalid
    helper-text="Your password is missing a number and a special character."
    :requirements="requirements"
  />
</div>`

const REQUIREMENTS_INVALID_SCRIPT = [
  "import { ref } from 'vue'",
  IMPORT,
  '',
...
/** @type {import('@storybook/vue3').StoryObj<typeof FieldPassword>} */
export const RequirementsInvalid = {
  render: () => ({
    components: { FieldPassword },
    setup() {
      const password = ref('abcdefgh')
    ...
    template: REQUIREMENTS_INVALID_TEMPLATE
  }),
  parameters: {
    docs: {
      controls: { disable: true },
      description: {
        story:
          'The error surface, for after a submit that failed. Setting `invalid` moves every rule the value does not satisfy to the danger treatment with an X glyph, while satisfied rules stay green: the reader sees which rules blocked the submit, not just that something did. Each of the three glyphs is a prop (`requirements-icon`, `requirements-pending-icon`, `requirements-invalid-icon`), so the set can be swapped without touching the component. Type in the field to watch chips leave the error state as their rule starts passing.'
      },
      source: { code: toSfc(REQUIREMENTS_INVALID_SCRIPT, REQUIREMENTS_INVALID_TEMPLATE) }
    }
  }
}
