...
import {
  ...
  resetDeployRuns,
  startDeployRun,
} from "../lib/deploy-runs";
import AppLayout from "./ui/AppLayout.vue";
import DeploymentFlow from "./ui/DeploymentFlow.vue";
import PageHeading from "./ui/PageHeading.vue";
import { logIntervalFor } from "./ui/deployment-steps.js";
...
<template>
  ...
  >
    <!-- The Deployments module's own column (the DATA measure), since this is a
         page INSIDE the module. It is one of the Forms flows, so the rhythm sits
         on the BAND (`.layout-section-start` = --layout-boundary-start, the same
         step the boundary puts above the heading), not on the container — which
         also happens to be what this page needs: its bands are conditional
         (idle EmptyState vs. outcome banner + timeline), so every band after the
    ...
    <main class="layout-column flex min-h-full flex-col">
      ...
      >
        <template #actions>
          <Tag :label="statusTag.label" :severity="statusTag.severity" size="medium" />
        </template>
      </PageHeading>
      ...
      >
        <div class="flex flex-wrap items-center justify-between gap-[var(--spacing-sm)]">
          ...
          <Tag :label="`${REPO.scope}/${REPO.name}`" severity="secondary" size="medium" />
        </div>
        <p class="m-0 text-body-sm text-[var(--text-muted)]">
          The ending is held in the URL (<code class="text-label-code-sm">?outcome=</code>),
          so a run is linkable and survives a reload. Pick one, press Deploy, and
          leave the page — the toast follows you and settles wherever you are.
        </p>
      ...
      >
        <template #actions>
          <Button label="Deploy" kind="primary" size="medium" @click="start" />
        </template>
      </EmptyState>
