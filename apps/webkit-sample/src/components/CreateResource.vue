<script setup>
  ...
  const submit = async () => {
    ...
    try {
      ...
      // action opens the surface that owns it, which is the whole reason someone reads
      // a success toast. Without it the reader lands on a list and has to find the row
      // they just made.
      toast.success(`${name} created.`, {
        action: {
          label: `Open ${spec.value.listLabel}`,
          onClick: () =>
            router.push({ path: spec.value.listPath, query: { email: userEmail.value } })
        }
      })

      // Back to the surface that owns the resource, carrying what was created so that
      // surface can show it without a round-trip. Overview reads `domain` to seed the
      // row that turns a first access into the returning dashboard
      // (see HomeEmptyState.vue) — the one thing the prototype has to fake. `workload`
      // rides along only when one was chosen: it is optional here, and a row that named
      // a workload the reader never picked would invent the binding this page is careful
      // not to force.
      ...
      router.push({
        ...
        query: {
          ...
          ...(props.resource === 'domains'
            ? { domain: name, ...(form.workload ? { workload: form.workload } : {}) }
            : {})
        }
      })
    } catch (error) {
