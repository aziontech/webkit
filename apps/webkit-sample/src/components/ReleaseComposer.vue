<script setup>
  ...
  import DeploymentSettingsPicker from './ui/DeploymentSettingsPicker.vue'
  import DeployProgressDialog from './ui/DeployProgressDialog.vue'
  import ImpactPanel from './ui/ImpactPanel.vue'
  import PageHeading from './ui/PageHeading.vue'
  import ReleaseDependenciesSection from './ui/ReleaseDependenciesSection.vue'
  import ReleaseTopologyTree from './ui/ReleaseTopologyTree.vue'
  ...
  const finishSingle = async () => {
    const settings = selectedRecords.value[0]
    deploying.value = true
    await sleep(900)
    deploying.value = false
    toast.success('Release deployed.', { description: settings.name })
    router.push({ path: '/deployments', query: { email: userEmail.value } })
  }

  const runTarget = async (settings, index) => {
    await sleep(600 + index * 500)
    const item = progressItems.value.find((entry) => entry.id === settings.id)
    if (!item) return
    if (failsFirstAttempt(settings)) {
      item.status = 'failed'
      item.message = DEPLOY_FAILURE_MESSAGE
      return
    }
    item.status = 'done'
    item.message = ''
  ...
  const runFanOut = async (records) => {
    deploying.value = true
    await Promise.all(records.map((settings, index) => runTarget(settings, index)))
    deploying.value = false
  }

  const deployMany = async () => {
    progressItems.value = selectedRecords.value.map((settings) => ({
      id: settings.id,
      name: settings.name,
      status: 'deploying',
      message: '',
      environments: settings.environmentNames.join(', ') || 'No workloads bound yet'
    }))
    progressOpen.value = true
    await runFanOut(selectedRecords.value)
  }

  const retryFailed = async () => {
    const failed = progressItems.value.filter((item) => item.status === 'failed')
    retriedIds.value = [...retriedIds.value, ...failed.map((item) => item.id)]
    failed.forEach((item) => {
      item.status = 'deploying'
      item.message = ''
    })
    await runFanOut(failed.map((item) => settingsById(item.id)).filter(Boolean))
  }
