<script setup>
  ...
  const recordName = computed(() => String(route.query.name || `${spec.value.unit} ${recordId.value}`))
  ...
  const validate = () => {
    ...
    for (const field of askedFields.value) {
      ...
      if (field.minLength && text.length < field.minLength) {
        errors[field.id] = { kind: 'invalid', message: `Use at least ${field.minLength} characters.` }
        continue
      }
      if (field.maxLength && text.length > field.maxLength) {
        errors[field.id] = { kind: 'invalid', message: `Use at most ${field.maxLength} characters.` }
        continue
      }
