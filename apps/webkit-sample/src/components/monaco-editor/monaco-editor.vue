...
<template>
  <div
    ...
    :data-fill="fill || null"
    class="flex w-full flex-col gap-[var(--spacing-xs)] data-[fill]:min-h-0 data-[fill]:flex-1"
  >
    ...
    <div
      ...
      class="min-w-0 h-[var(--monaco-editor-height)] data-[fill]:h-auto data-[fill]:min-h-0 data-[fill]:flex-1 overflow-hidden rounded-[var(--shape-elements)] border border-[var(--border-default)] bg-[var(--bg-surface)] transition-colors duration-150 ease-out motion-reduce:transition-none focus-within:outline-none focus-within:ring-2 focus-within:ring-[var(--ring-color)] focus-within:ring-offset-2 focus-within:ring-offset-[var(--bg-canvas)] data-[invalid]:border-[var(--danger-border)] data-[disabled]:cursor-not-allowed data-[disabled]:bg-[var(--bg-disabled)]"
    >
      <VueMonacoEditor
        v-model:value="model"
        :language="language"
