<script setup>
  // Animated ASCII-art banner rendered to a <canvas>, used as a hero backdrop.
  // A scene is sampled onto a monospace character grid: base geometry renders as
  // faint gray glyphs, node anchors pulse in the brand accent, and travelling
  // "packets" sweep along arcs/edges/contour lines. The canvas is transparent (it
  // clears each frame) so it layers over the section's own background.
  //
  // Three interchangeable scenes, selected via the `scene` prop:
  //   • globe   — a rotating wireframe sphere with great-circle arcs (edge network).
  //   • mesh    — drifting nodes joined by proximity edges with pulsing packets.
  //   • contour — an animated topographic field of interfering sine waves.
  //
  ...
  const props = defineProps({
    ...
    scene: {
      type: String,
      default: 'contour',
      validator: (v) => ['globe', 'mesh', 'contour'].includes(v)
    },
    ...
    /** Accent glow intensity (shadow blur multiplier). */
    glow: { type: Number, default: 0.6 },
    /** Number of pulsing node anchors (globe/mesh). */
    nodes: { type: Number, default: 22 },
    /** Number of arcs/packets sweeping between nodes (globe). */
    arcs: { type: Number, default: 10 },
  ...
  function build() {
    ...
    // Globe node anchors (unit vectors) via a Fibonacci-sphere distribution.
    ...
    // Mesh nodes drifting in normalized [0,1] space.
    meshN = []
    for (let m = 0; m < props.nodes; m++) {
      meshN.push({
        x: rnd(),
        y: rnd(),
        vx: (rnd() - 0.5) * 0.00006,
  ...
  function mesh(t) {
    const pad = 0.06
    for (let i = 0; i < meshN.length; i++) {
      const n = meshN[i]
      n.x += n.vx * (1 + props.speed)
      n.y += n.vy * (1 + props.speed)
      if (n.x < pad || n.x > 1 - pad) n.vx *= -1
      if (n.y < pad || n.y > 1 - pad) n.vy *= -1
    }

    // Proximity edges with a travelling packet.
    const thr = 0.24
    for (let i = 0; i < meshN.length; i++) {
      for (let j = i + 1; j < meshN.length; j++) {
        const a = meshN[i]
        const b = meshN[j]
        const dx = a.x - b.x
        const dy = a.y - b.y
        ...
        const base = 0.28 * (1 - dist / thr) + 0.08
        const head = (t * 0.0008 * props.speed + ((i * j) % 7) / 7) % 1
        for (let s = 0; s <= steps; s++) {
          const f = s / steps
          const x = (a.x + (b.x - a.x) * f) * W
          const y = (a.y + (b.y - a.y) * f) * H
          let d = Math.abs(f - head)
          d = Math.min(d, 1 - d)
          const pulse = Math.max(0, 1 - d * 9)
          put(x, y, base + 0.6 * pulse, pulse > 0.3 ? 2 : 0)
        }
      }
    }

    for (let i = 0; i < meshN.length; i++) {
      const pulse = 0.75 + 0.25 * Math.sin(t * 0.004 + i)
      put(meshN[i].x * W, meshN[i].y * H, pulse, 1)
    }
  }

  function contour(t) {
    const tt = t * 0.0006 * props.speed
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        let v =
          Math.sin(nx * 6.2 + tt) +
          Math.sin(ny * 5.1 - tt * 0.8) +
          Math.sin((nx + ny) * 4.0 + tt * 0.6) +
          Math.sin((nx - ny) * 7.3 - tt * 0.4)
        v = (v + 4) / 8 // 0..1
        const bands = 7
        const d = Math.abs(((v * bands) % 1) - 0.5) * 2 // 0 at a contour line
        const b = Math.pow(1 - d, 3)
        const idx = row * cols + col
        bright[idx] = b * 0.9
        kind[idx] = b > 0.72 ? 2 : 0
      }
  ...
  function renderScene(t) {
    if (props.scene === 'globe') globe(t)
    else if (props.scene === 'mesh') mesh(t)
    else contour(t)
  }
