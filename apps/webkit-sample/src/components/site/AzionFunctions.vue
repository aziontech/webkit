<script setup>
  ...
  import { useScrollProgress } from './foundations/composables/useScrollProgress.js'
  ...
  import { PixelateBanner } from './ui/banners/index.js'
  ...
  import contabilizeiTile from './ui/clients/contabilizei-symbol.png'
  ...
  <SectionContainer max-width="5xl">
    ...
    >
      ...
      <!-- Registration-framed band, `y` only: the menu's side edges are the column's own
           border-x, so drawing them here would double them. The top rule divides the menu
           from the headline block, which the `#header` slot does not draw. -->
      <FrameBox borders="y">
        <CardGrid
          variant="divider"
          :columns="4"
