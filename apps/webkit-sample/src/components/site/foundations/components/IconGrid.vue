...
<template>
  <div class="flex flex-col gap-8">
    ...
    <div v-if="azionIcons.length > 0" class="flex flex-col gap-3">
      <div class="flex items-baseline justify-between gap-[var(--spacing-sm)] px-[var(--spacing-xxs)]">
        <h3 class="m-0 text-overline-sm uppercase tracking-widest text-[var(--text-muted)]">Azion Icons</h3>
        <span class="text-body-xs text-[var(--text-muted)]">{{ azionIcons.length }} / {{ totalAzion }}</span>
      </div>
      <p class="m-0 mb-2 text-body-xs text-[var(--text-muted)]">
        Custom Azion product and technology icons.
      </p>
      
      <div class="grid grid-cols-4 gap-0 sm:grid-cols-6 lg:grid-cols-8">
        <button
    ...
    <div v-if="filteredColorIcons.length > 0" class="flex flex-col gap-3">
      <div class="flex items-baseline justify-between gap-[var(--spacing-sm)] px-[var(--spacing-xxs)]">
        <h3 class="m-0 text-overline-sm uppercase tracking-widest text-[var(--text-muted)]">Colored Icons</h3>
        <span class="text-body-xs text-[var(--text-muted)]">{{ filteredColorIcons.length }} / {{ colorIcons.length }}</span>
      </div>
      <p class="m-0 mb-2 text-body-xs text-[var(--text-muted)]">
    ...
    <div v-if="primeIcons.length > 0" class="flex flex-col gap-3">
      <div class="flex items-baseline justify-between gap-[var(--spacing-sm)] px-[var(--spacing-xxs)]">
        <h3 class="m-0 text-overline-sm uppercase tracking-widest text-[var(--text-muted)]">PrimeIcons</h3>
        <span class="text-body-xs text-[var(--text-muted)]">{{ primeIcons.length }} / {{ totalPrime }}</span>
      </div>
      <p class="m-0 mb-2 text-body-xs text-[var(--text-muted)]">
