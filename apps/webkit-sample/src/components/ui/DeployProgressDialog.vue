<script setup>
  ...
  // heading a reader trusts and then gets burned by.
  const title = computed(() => {
    const { total, done, failed, settled } = counts.value
    const noun = total === 1 ? 'Deployment setting' : 'Deployment settings'
    if (running.value) return `Deploying ${settled} of ${total}…`
    if (failed) return `Deployed ${done} of ${total} ${noun}`
        ...
        <PanelHeader class="w-full">
          <div class="flex min-w-0 flex-col gap-[var(--spacing-xxs)]">
            ...
            <p class="text-body-sm text-[var(--text-muted)]">
              {{ counts.done }} succeeded, {{ counts.failed }} failed. Each Deployment setting
              activates on its own, and the ones that succeeded stay live.
            </p>
          </div>
        </PanelHeader>
