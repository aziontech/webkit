<script setup>
  ...
  import { SAMPLE_MODES } from '../../lib/sample-mode'
  import { SAMPLE_PLANS, useSamplePreset } from '../../lib/sample-preset'
  ...
  const MODE_DESCRIPTIONS = {
    empty: 'Home opens on first use; Applications, Workloads and Functions show their first-use block.',
    populated: 'Home opens on usage and the resource list; every module lists its seeded rows.'
  }

  const modeOptions = computed(() =>
    SAMPLE_MODES.map((option) => ({ ...option, description: MODE_DESCRIPTIONS[option.value] ?? '' }))
  )
        ...
        <PanelContent>
          <div class="flex flex-col gap-[var(--spacing-lg)]">
            <DrawerDescription class="m-0 text-body-sm text-[var(--text-muted)]">
              Which customer this prototype is pretending to be. Nothing here exists in
              the real console — it changes the screens beside this panel as you set it,
              is remembered for the next session, and can be handed to somebody else as a
              link.
            </DrawerDescription>
