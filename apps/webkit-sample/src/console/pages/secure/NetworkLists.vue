<script setup>
  // Network Lists — the Azion Console "Network Lists" module. The app shell (single sidebar +
  ...
  // against. Its TYPE decides what the entries even are — IP/CIDR ranges, ASNs, or
  // country codes — so it is the column that leads and the field that matters.
  //
  ...
  import { NETWORK_LISTS, networkListTypeOptions } from '../../lib/data/network-lists'
  ...
  const firstUse = productFirstUse('network-lists')
  ...
  // A network list belongs to one place in the tenancy chain, so the seed is projected
  ...
  const columns = [
    ...
    { accessorKey: 'entries', header: 'Entries', enableSorting: true, minWidth: FIT_COLUMN },
  ...
  // ── The filter catalog ────────────────────────────────────────────────────
  // Type and Status are the enumerable columns; Entries is a magnitude and Name is
  // free text, covered by the search field.
  const filterFields = [
    {
      id: 'type',
      label: 'Type',
      kind: 'options',
      options: networkListTypeOptions,
      match: (list, values) => values.includes(list.type)
    },
    {
      id: 'status',
      label: 'Status',
      ...
      match: (list, values) => values.includes(list.status)
    },
    {
      id: 'modified',
      label: 'Last Modified',
      ...
      match: (list, values) => matchDate(list.modifiedAt, values)
    }
  ]
  ...
  const {
    ...
    search,
    pagination,
    visibleRows: visibleLists,
    loading,
    refresh
  } = useListFilters(filterFields, scopedLists, { pageSize: 8 })
  ...
  const create = () =>
    router.push({
      path: createResourcePath('network-lists'),
  ...
  const confirmDelete = () => {
    ...
    removeCreatedResource('network-lists', row.id)
    lists.value = lists.value.filter((item) => item.id !== row.id)
  ...
  const onRowAction = (event, action, row) => {
    ...
    // left a reader able to create a network lists and unable to change one. The page is
    ...
    if (action === 'edit') {
      router.push({
        path: resourceSettingsPath('network-lists', row.id),
...
<template>
  <AppLayout
    active="network-lists"
    :breadcrumb="[{ label: 'Network Lists' }]"
  >
    <main
    ...
    >
      ...
      >
        ...
        <section class="flex min-w-0 flex-col gap-(--layout-group-gap)">
          <ControlsHeader v-if="scopedLists.length">
            <FilterButton
              v-model="filters"
              :fields="filterFields"
            />
            <InputText
              v-model="search"
              size="medium"
              placeholder="Search network lists"
              aria-label="Search network lists"
            ...
            <CardBox class="w-full max-w-(--container-2xl)">
              <template #content>
                <EmptyState
                  ...
                  description="Create a network list to allow or deny traffic by IP range, autonomous system or country."
                ...
                >
                  <template #icon>
                    <span class="relative flex size-10 items-center justify-center">
                      ...
                      >
                        <i
                          class="ai ai-network-lists text-[1rem] leading-none text-(--text-default)"
                          aria-hidden="true"
                        />
                      </span>
                    </span>
