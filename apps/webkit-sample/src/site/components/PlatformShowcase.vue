...
<template>
  ...
  >
    ...
    <!-- The band is a registration frame like every other brick in the column, so the
         four panels sit inside one marked box instead of floating between their
         neighbours. `borders="y"` hands the vertical rules back to the column, `flush`
         leaves the rule above to the SectionTitle in the `#header` slot, and
         `marks="bottom"` ticks the one junction nothing else draws — this band's floor. -->
    <FrameBox
      flush
      borders="y"
      marks="bottom"
