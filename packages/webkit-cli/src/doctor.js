...
export function planDoctor(projectDir) {
  ...
  add('eslint config', eslintCfg ? 'ok' : 'fail', eslintCfg || 'no eslint config found — run `webkit-cli init`.')
  ...
  add('stylelint config', hasStylelint ? 'ok' : 'fail', stylelintCfg || (pkg && pkg.stylelint ? 'package.json#stylelint' : 'no stylelint config found — run `webkit-cli init`.'))
  ...
  if (!existsSync(mcpPath)) {
    ...
    try {
      ...
      add('mcp server', server ? 'ok' : 'fail', server ? `"${MCP_SERVER_NAME}" registered` : `.mcp.json has no "${MCP_SERVER_NAME}" server.`)
    } catch (err) {
  ...
  add('husky prepare script', prepare === 'husky' ? 'ok' : 'warn', prepare === 'husky' ? 'scripts.prepare = husky' : 'missing scripts.prepare="husky" — git hooks will not activate on install.')
  ...
  add('pre-commit hook', hasHook ? 'ok' : 'warn', hasHook ? '.husky/pre-commit lints on commit' : 'no .husky/pre-commit lint block — commits are not linted locally.')
  ...
  if (entry) {
    ...
    add('theme import', themed ? 'ok' : 'warn', themed ? `imported in ${entry}` : `add \`import '@aziontech/theme'\` to ${entry}.`)
  }
  ...
  if (present.length) {
    ...
    if (floating.length) {
      const pins = floating
        ...
        .join(', ')
      add('dependency versions', 'warn', `${floating.length} pinned as "latest" — consider pinning: ${pins}.\n${lines.join('\n')}`)
    } else {
