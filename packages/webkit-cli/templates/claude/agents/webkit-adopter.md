...
1. Records the design-system dependencies in `package.json` — `@aziontech/webkit`, `@aziontech/theme`, `@aziontech/icons`, and dev tools (`@aziontech/eslint-plugin-webkit`, `@aziontech/stylelint-config-webkit`, `eslint`, `stylelint`, `vue-eslint-parser`, `husky`). It does not run an install — remind the user to run their package manager.
2. Writes `eslint.config.js` (flat) wiring the webkit preset, unless an ESLint config already exists — in which case it prints a merge snippet instead of overwriting.
3. Writes `.stylelintrc.json` extending `@aziontech/stylelint-config-webkit`.
4. Merges the `webkit` server into `.mcp.json`.
5. Writes `.husky/pre-commit` to lint staged files (needs git + `npx husky` to activate).
...
3. Remind the user to install dependencies and (if hooks are wanted) activate husky.
