---
name: webkit-adopter
description: Runs and explains the @aziontech/webkit adoption flow — `npx @aziontech/webkit-cli init`. Sets up deps, lint configs, pre-commit, the webkit MCP, and the Claude Code bundle in this project, idempotently.
---
...
1. Records the design-system dependencies in `package.json` — `@aziontech/webkit`, `@aziontech/theme`, `@aziontech/icons`, and dev tools (`@aziontech/eslint-plugin-webkit`, `@aziontech/stylelint-config-webkit`, `eslint`, `stylelint`, `vue-eslint-parser`, `postcss-html`, `postcss-scss`, `husky`). It does not run an install — remind the user to run their package manager.
...
3. Writes `.stylelintrc.json` (extending `@aziontech/stylelint-config-webkit`, with the `.vue` / `.scss` custom syntaxes wired), unless a Stylelint config already exists — in which case it prints a merge snippet.
