<script setup lang="ts">
   ...
   * without any page having to rebuild the row, the order, the rhythm or the type
   * scale to say one thing differently:
   *
   * - `breadcrumb` over the `breadcrumb` prop — the built-in renders the same
   *   component, but an application's trail must `router.push` instead of loading
   *   the document, so it passes the wired control in.
   * - `title` over the `title` prop — a prose page's name is a string; a page ABOUT
   *   something (a tool, a vendor, a product) is an identity: a mark beside the
   *   name and the maker above it. That is a title, not a decoration, so it goes
   *   where the `h1` goes rather than into a band of its own above the masthead.
   ...
   * what it will do to the reader's day. The tip is that sentence, on hover and on
   * focus, from the design system's own tooltip rather than a `title` attribute
   * that no touch device and no keyboard ever shows.
   *
...
<template>
  ...
  >
    ...
    <slot name="details" />
    <!-- THE META LINE: when the page changed, then what can be done with it. One row,
         so the date reads as the first entry in the page's utility belt rather than as a
         caption of its own — and it wraps per control, because on a phone the strip is
