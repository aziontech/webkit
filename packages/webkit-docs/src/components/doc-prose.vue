<script setup lang="ts">
   ...
   *
   * THE `pt` HALF OF THAT RESET SPARES CHROME, the `mt` half does not. For a
   * paragraph or a heading, `pt` *is* the space above the block, so zeroing it
   * is the whole point. For a component that brings its own box — a callout, a
   * card, a prompt — `pt` is its INTERNAL top inset, and zeroing it flattened
   * the component against its own border: a page opening with `<Warning>`
   * rendered a callout with `padding-top: 0` and `padding-bottom: 6px`, its copy
   * pressed to the top rule while the space sat underneath. The same callout
   * further down the page was a correct 6/6, which is what made it look like a
   * bug in the callout rather than in the prose around it. Chrome spaces itself
   * with `mt` (`[&_[data-doc-block]]:mt-(--spacing-lg)`), so `mt-0!` still
   * flushes a first block correctly and there is nothing for `pt-0!` to do
   * there but damage.
   *
   ...
   * Paragraphs and list items are the muted voice, and the things a reader
   ...
   * and axe flags it once per paragraph. The fix, if it is wanted, is a docs
   * body ink one step darker rather than a different grey: `#737373` reads as
   * the same quieter voice and clears AA at 4.54:1 on canvas.
   *
   ...
   * corners are `--shape-elements` (6px), not the `--shape-flat` they used to
   * be: a square-cornered tint reads as a highlighter stroke over the
   * sentence, where a rounded one reads as a discrete token sitting in it.
   * The same radius is on the code chips in DocCallout and DocItem, and on
   * the tool-name chips in the agent page's MCP table, so every chip a reader
   * can copy has one shape.
   *
   ...
   * command, a flag, a tool name — so the chip that marks it may as well hand it
   * over. Hovering one shows "Copy"; clicking copies its text and the label
   * turns to "Copied".
   *
   * DELEGATED FROM THIS ROOT, not built into a chip component. Prose `code`
   * arrives three ways — a vnode from the MDX renderer, hand-written markup in a
   * `.vue` page, raw HTML from another pipeline — and only two of those can hold
   * a component. A listener on the container catches all three, and it is also
   * where the chip is already styled, so presentation and behaviour stay in one
   * file. Attached programmatically rather than as a template `@click`: a
   * `@click` on this non-interactive root is what `click-events-have-key-events`
   * exists to stop.
   *
   * IT ADDS NO TAB STOPS, deliberately. Making every chip a real button would
   * put thirty of them on a docs page in front of a keyboard user, for a string
   * that is already plain selectable text — the same trade that took the nine
   * copy buttons out of the agent page's tool table. The copy is a pointer
   ...
   * control) and anything inside `[data-doc-chrome]` (a component's own
   * internals), matching the styling rules above.
   */
  const proseRef = ref<HTMLElement | null>(null)
  const tipFor = ref<HTMLElement | null>(null)
  const tipLabel = ref<'Copy' | 'Copied' | 'Press ⌘C to copy'>('Copy')
  const tipLeft = ref(0)
  const tipTop = ref(0)
  ...
  const chipFrom = (target: globalThis.EventTarget | null): HTMLElement | null => {
    ...
    // WHAT COUNTS AS A CHIP is not `[data-doc-chrome]`, the boundary the STYLING uses.
    // A chip in a callout, in an item row, or in the agent page's tool table sits
    // inside chrome and is still a literal the reader wants — it looks identical to a
    // chip in a paragraph, so it has to behave identically. The line that actually
    // matters is authored-vs-generated, and the DOM already draws it: a `code` a
    // component owns identifies itself with a `data-testid` and holds child elements
    // (a highlighted CodeBlock line is nine token spans), where an authored chip is a
    // bare element with one text node. Those two tests keep every code-block line out
    // — they have their own copy control — and let every real chip in.
...
<template>
  <div
    ...
    class="w-full text-(--text-default) [&>*:first-child]:mt-0! [&>*:first-child:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-0! [&>*:first-child>*:first-child]:mt-0! [&>*:first-child>*:first-child:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-0! [&_h1:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-14 [&_h1:not([data-doc-chrome],[data-doc-chrome]_*)]:text-heading-xl [&_h1:not([data-doc-chrome],[data-doc-chrome]_*)]:max-sm:[font-size:var(--text-2xl)] [&_h1:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-default) [&_h2:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-14 [&_h2:not([data-doc-chrome],[data-doc-chrome]_*)]:text-heading-xl [&_h2:not([data-doc-chrome],[data-doc-chrome]_*)]:sm:text-heading-md [&_h2:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-default) [&_h3:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-14 [&_h3:not([data-doc-chrome],[data-doc-chrome]_*)]:text-heading-lg [&_h3:not([data-doc-chrome],[data-doc-chrome]_*)]:md:text-heading-sm [&_h3:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-default) [&_h4:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-xl) [&_h4:not([data-doc-chrome],[data-doc-chrome]_*)]:sm:pt-(--spacing-lg) [&_h4:not([data-doc-chrome],[data-doc-chrome]_*)]:text-heading-xs [&_h4:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-default) [&_p:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-md) [&_p:not([data-doc-chrome],[data-doc-chrome]_*)]:text-body-md [&_p:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-muted) [&_strong:not([data-doc-chrome],[data-doc-chrome]_*)]:font-normal [&_strong:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-default) [&_em:not([data-doc-chrome],[data-doc-chrome]_*)]:italic [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:rounded-(--shape-flat) [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-link) [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:underline [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:decoration-(--text-link)/40 [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:underline-offset-4 [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:transition-colors [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:duration-150 [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:ease-out [&_a:hover:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-link-hover) [&_a:hover:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:decoration-(--text-link-hover) [&_a:focus-visible:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:outline-2 [&_a:focus-visible:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:outline-offset-2 [&_a:focus-visible:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:outline-(--ring-color) [&_a:not([data-doc-anchor],[data-doc-chrome],[data-doc-chrome]_*)]:motion-reduce:transition-none [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:rounded-(--shape-elements) [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:border [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:border-(--border-default) [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:bg-(--bg-hover) [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:px-(--spacing-xs) [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:py-0.5 [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:text-label-code-sm [&_code:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-default) [&_code:not([data-testid])]:cursor-pointer [&_code:not([data-testid])]:hover:ring-1 [&_code:not([data-testid])]:hover:ring-(--border-default) [&_ul:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-md) [&_ol:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-md) [&_ul:not([data-doc-chrome],[data-doc-chrome]_*)]:list-disc [&_ol:not([data-doc-chrome],[data-doc-chrome]_*)]:list-decimal [&_ul:not([data-doc-chrome],[data-doc-chrome]_*)]:pl-(--spacing-lg) [&_ol:not([data-doc-chrome],[data-doc-chrome]_*)]:pl-(--spacing-lg) [&_li:not([data-doc-chrome],[data-doc-chrome]_*)]:text-body-md [&_li:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-muted) [&_li:not([data-doc-chrome],[data-doc-chrome]_*)]:marker:text-(--text-muted) [&_li+li:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-xs) [&_li>p:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-sm) [&_li_ul:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-sm) [&_li_ol:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-sm) [&_blockquote:not([data-doc-chrome],[data-doc-chrome]_*)]:mt-(--spacing-lg) [&_blockquote:not([data-doc-chrome],[data-doc-chrome]_*)]:border-l-2 [&_blockquote:not([data-doc-chrome],[data-doc-chrome]_*)]:border-(--border-strong) [&_blockquote:not([data-doc-chrome],[data-doc-chrome]_*)]:pl-(--spacing-lg) [&_blockquote_p:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-0 [&_blockquote_p:not([data-doc-chrome],[data-doc-chrome]_*)]:text-(--text-muted) [&_hr:not([data-doc-chrome],[data-doc-chrome]_*)]:mt-12 [&_hr:not([data-doc-chrome],[data-doc-chrome]_*)]:border-0 [&_hr:not([data-doc-chrome],[data-doc-chrome]_*)]:border-t [&_hr:not([data-doc-chrome],[data-doc-chrome]_*)]:border-(--border-default) [&_[data-doc-block]]:mt-(--spacing-lg) [&_:is(h1,h2)+:is(h2,h3,h4):not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-md) [&_h3+:is(h4,p,ul,ol,blockquote):not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-sm) [&_h4+:is(h4,p,ul,ol,blockquote):not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-xs) [&_[data-doc-block]+p:not([data-doc-chrome],[data-doc-chrome]_*)]:pt-(--spacing-lg) [&_h3+[data-doc-block]]:mt-(--spacing-sm) [&_h4+[data-doc-block]]:mt-(--spacing-xs)"
  >
    <slot />

    <!-- ONE panel for every chip on the page, moved to whichever is hovered. Per-chip
         tooltips would mean a component per code span, which the raw-HTML and
         hand-written paths cannot have (see the copy block in the script). It borrows
         the design system Tooltip's own surface — contrast fill, `--shape-elements`,
         `text-body-xs` — so a gloss in the docs and a tooltip in the console are the
         same object, and it is `pointer-events-none` so it can never sit between the
         pointer and the chip it describes.
    ...
    <Teleport to="body">
      ...
      >
        <span
          v-if="tipFor"
          role="tooltip"
          data-testid="doc-prose__copy-tip"
