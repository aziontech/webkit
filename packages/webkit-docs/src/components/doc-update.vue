<script setup lang="ts">
   ...
   * above the changelog.
   *
   * NO RSS. Mintlify's `<Update>` also takes an `rss` prop, because Mintlify
   * generates the feed. This layer renders pages and generates nothing, so the
   * prop would be inert — an entry in the API that quietly does nothing is worse
   * than an absence. Feed generation belongs to whatever builds the site.
   */
  defineOptions({ name: 'DocUpdate' })
