...
- Style with `@aziontech/theme` tokens (`var(--primary)`, `var(--spacing-md)`, `text-button-lg`) — never hex, `rgb`, `hsl`, or raw Tailwind palette.
- Never restyle a webkit component: no `class`/`:class`/`style` on its tag (lint blocks it as an error). Compose inside its slots, use its props, or use a `styleSeam` component — see `.claude/rules/webkit-style-override.md`.
- When building a component of your own, follow the construction standards in `.claude/rules/webkit-construction-standards.md` (`defineModel`, typed props/emits/slots, `data-*` styling, a11y).
