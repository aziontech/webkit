...
# Skill: webkit-delight

## Purpose

Find the _one_ moment where a touch of personality turns a functional interface into one users
...
- **Last**, after `/webkit-ux-heuristics` → `/webkit-baseline-ui` → `/webkit-motion-polish` →
  `/webkit-impeccable-polish` all pass.
...
- **Token-driven motion only** (see `/webkit-motion-polish`). A satisfying state change is a
  `transform`/`opacity` transition on the right curve — nothing more.
- **No new dependencies.** No `canvas-confetti`, `gsap`, `framer-motion`, lottie, sound libraries, or
  any package is allowed. No component-local `@keyframes`.
...
- Confetti / glow / multicolor gradients as a default celebration (also banned by `/webkit-baseline-ui`).
...
For `/webkit-delight <file>`, output two short lists:
...
- Motion tokens: the animation tokens (see `/webkit-motion-polish`).
- No-lib rule: no new dependency, no component-local `@keyframes`.
- Restraint baseline: `/webkit-baseline-ui` (no gradient/glow unless requested).
