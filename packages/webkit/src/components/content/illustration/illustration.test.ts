...
describe('Illustration (compound / provide-inject)', () => {
  ...
  it('runs the chat scene as a transcript that scrolls over a thinking row that does not', () => {
    const view = render(IllustrationWindow, { props: { kind: 'chat' } })
    // children[0] is the chrome (the three dots); children[1] is the scene.
    const [transcript, thinking] = [...part(view, 'window').children[1].children]

    // One screenful of conversation rendered twice on the track the scroll moves. The copy is
    // what makes the one-way loop seamless — half a track of travel has to land on an identical
    // frame — so the two halves must stay identical, bubble for bubble.
    const bubbles = [...(transcript.firstElementChild?.children ?? [])] as HTMLElement[]
    expect(bubbles.length).toBe(8)
    const shape = (b: HTMLElement) => [b.className, b.style.animationDelay]
    expect(bubbles.slice(0, 4).map(shape)).toEqual(bubbles.slice(4).map(shape))
    // …and the four within a screenful must differ, or it is a placeholder, not a dialogue.
    expect(new Set(bubbles.slice(0, 4).map((b) => shape(b).join('|'))).size).toBe(4)

    // The thinking row is a sibling of the transcript, not a passenger on the track: the
    // assistant stays put working on the next answer while the conversation moves past it.
    expect(transcript.contains(thinking)).toBe(false)
    // Its two parts: the spinner ring and the bar that pulses beside it.
    expect(thinking.children.length).toBe(2)
  })
  ...
  it('fills the gauge arc in proportion to its value and clamps out of range', () => {
    // `pathLength="100"` renormalizes the circumference, so the dash IS the value.
    const arcFor = (value: number) => {
      const view = render(IllustrationGauge, { props: { value } })
      const arc = part(view, 'gauge').querySelectorAll('circle')[1] as SVGCircleElement
      return Number.parseFloat(globalThis.getComputedStyle(arc).strokeDasharray)
    }
    expect(arcFor(0)).toBeCloseTo(0)
    expect(arcFor(50)).toBeCloseTo(50)
    expect(arcFor(100)).toBeCloseTo(100)
    // Out-of-range values clamp rather than drawing a second lap or a negative arc.
    expect(arcFor(140)).toBeCloseTo(100)
    expect(arcFor(-20)).toBeCloseTo(0)
  })

  it('lets a scene raise the gauge value, and drives arc and readout from the same number', () => {
    // The prop is the resting value; `--illustration-gauge-target` from anywhere above wins,
    // which is what a hovering card sets to count the score up.
    const resting = render(IllustrationGauge, { props: { value: 72, showValue: true } })
    const gauge = part(resting, 'gauge')
    expect(
      globalThis.getComputedStyle(gauge).getPropertyValue('--illustration-gauge-value').trim()
    ).toBe('72')

    const raised = render(IllustrationGauge, {
      props: { value: 72, showValue: true },
      attrs: { style: '--illustration-gauge-target: 100' }
    })
    const raisedGauge = part(raised, 'gauge')
    expect(
      globalThis.getComputedStyle(raisedGauge).getPropertyValue('--illustration-gauge-value').trim()
    ).toBe('100')
    const arc = raisedGauge.querySelectorAll('circle')[1] as SVGCircleElement
    expect(Number.parseFloat(globalThis.getComputedStyle(arc).strokeDasharray)).toBeCloseTo(100)
  })

  it('prints the value instead of the label when showValue is set', () => {
    const view = render(IllustrationGauge, {
      props: { value: 72, label: 'ignored', showValue: true }
    })
    const gauge = part(view, 'gauge')
    // The digits come from a CSS counter on ::before, so the span itself stays empty.
    expect(gauge.textContent?.trim()).toBe('')
    expect(gauge.querySelector('[data-testid="content-illustration-gauge__value"]')).not.toBeNull()
  })
