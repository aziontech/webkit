...
<template>
  ...
  >
    ...
    >
      ...
      >
        <!-- Two treatments, matching Figma, nothing configurable: a satisfied rule gets the
             success fill plus the check glyph, an unsatisfied one the placeholder fill and
             no visible glyph. The glyph stays in the DOM so its box can animate from zero
             width: unsatisfied it reserves no space; satisfied it widens to 14px, gains the
             gap as its own margin, and slides in from the left. -->
        <i
          :data-validated="requirement.met || null"
