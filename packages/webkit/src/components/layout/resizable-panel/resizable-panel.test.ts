...
import ResizablePanel, { ResizablePanelHandle, ResizablePanelPane } from './index'
...
const group = (
  ...
  h(ResizablePanel, null, {
    default: () => [
      h(ResizablePanelPane, null, { default: () => 'document' }),
      h(ResizablePanelHandle, { 'aria-label': 'Resize the preview', ...handleProps }),
      h(
        ResizablePanelPane,
...
describe('ResizablePanel', () => {
  ...
  // The one place dot-notation is the subject rather than the vehicle: the parts the group
  // above renders are the same objects the root exposes, so `<ResizablePanel.Pane>` in a
  // consumer's template resolves to what this file exercises. Read through `?.` because the
  // root's default export is generated by the SFC compiler — static analysis sees no
  // `export default` in a `.vue`, reads the binding as undefined, and flags every unguarded
  // member access on it.
  it('attaches its sub-components to the root for dot notation', () => {
    expect(ResizablePanel).toBeTruthy()
    expect(ResizablePanel?.Pane).toBe(ResizablePanelPane)
    expect(ResizablePanel?.Handle).toBe(ResizablePanelHandle)
  })
  ...
  it('flips the separator orientation with the group axis', () => {
    const { getByRole } = render(
      h(
        ...
        {
          default: () => [
            h(ResizablePanelPane, null, { default: () => 'editor' }),
            h(ResizablePanelHandle, { 'aria-label': 'Resize the terminal' }),
            h(ResizablePanelPane, { basis: 176 }, { default: () => 'terminal' })
          ]
        }
      )
    )
