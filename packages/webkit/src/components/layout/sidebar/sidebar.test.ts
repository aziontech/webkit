...
describe('Sidebar', () => {
  ...
  describe('side prop (a trailing rail is the same component)', () => {
    ...
    it('a collapsed trailing rail publishes its edge on the panel, and holds no tab stops', async () => {
      const { getByTestId } = render(Sidebar, {
      ...
      expect(root.getAttribute('aria-hidden')).toBe('true')
      expect(getByTestId('layout-sidebar__panel').getAttribute('data-side')).toBe('end')
    })
