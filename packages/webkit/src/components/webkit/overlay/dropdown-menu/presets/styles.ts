...
export const dropdownMenuContentClasses = [
  ...
  'bg-[var(--bg-surface)] p-[var(--spacing-1)] shadow-[var(--shadow-md)]',
  'outline-none'
] as const

export const dropdownMenuGroupClasses = [
  'flex h-7 items-center px-[var(--spacing-2)] py-[var(--spacing-1)]',
  'text-overline-xs uppercase text-[var(--text-muted)]'
] as const

const dropdownMenuItemSharedClasses = [
  'relative flex w-full min-h-8 cursor-pointer items-center gap-[var(--spacing-2)]',
  'rounded-[var(--shape-elements)] px-[var(--spacing-2)] text-left text-label-md',
...
export const dropdownMenuSeparatorClasses = [
  'my-[var(--spacing-1)] h-px w-full shrink-0 bg-[var(--border-muted)]'
] as const
